...
if (window._commonLoaded) {
    ...
    window.syncUserAvatar = async function () {
        ...
        if (!avatarUrl || avatarUrl === 'https://www.gravatar.com/avatar/00000000000000000000000000000000?d=mp') {
            if (user.email) {
                // 使用 blueimp-md5（已在页面中加载）
                const emailMd5 = md5 ? md5(user.email) : '';
                if (emailMd5) avatarUrl = `https://www.gravatar.com/avatar/${emailMd5}?d=mp&s=200`;
            }
        }
    ...
    window.showNotificationBar = function (msg) {
        ...
        notifTimeout = setTimeout(() => { bar.style.display = 'none'; }, 5000);
    };
    ...
    window.showQuickReplyModal = function (animeId, commentId, replierEmail, originalReplyText) {
        const modal = document.createElement('div');
        modal.style.cssText = `
            position: fixed;
            top: 50%;
    ...
    window.loadChatHistory = async function () {
        ...
        const res = await fetch(`/api/messages/history?friend=${encodeURIComponent(currentChatFriend)}`, { credentials: 'include' });
        if (!res.ok) return;
    ...
    window.sendMessage = async function () {
        ...
        const res = await authFetch('/api/messages/send', { method: 'POST', body: { to: currentChatFriend, text } });
        if (res.ok) {
            input.value = '';
            await loadChatHistory();
        } else showToast('发送失败');
